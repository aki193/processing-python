...
# 60枚の画像読み込み
for i in range(1, 10):
	img0rigin[i] = cv2.imread("data/pic0"+str(i)+".jpg")

for i in range(10, 61):
	img0rigin[i] = cv2.imread("data/pic"+str(i)+".jpg")
    
#for i in range(61):
#	img0rigin_sum[i].append(img0rigin[i])
...
for i in range(1, 61):
    # グレースケールへ変換後に２値化する
    imgGray = cv2.cvtColor(imgOrigin[i],cv2.COLOR_BGR2GRAY)
...
# 取得した輪郭に対して処理を行う（輪郭は複数取れている）
    for contour in contours:
        ...
        imgContour = cv2.drawContours(imgOrigin[i], contour, -1, (0, 255, 0), 3)
        # 輪郭の面積を算出
